...
int mysender_test(char *message, char *filename, char* window_size, char* prob, char* err)
{
    //std::cout<<"sender test"<<std::endl;
    std::thread receiver_thread(normal_receiver, filename, window_size);
    usleep(10000);
...
TEST(RTP, SENDER_MYTEST)
{
    ...
    char* pb = (char*)"10";
    char* er = (char*)"1";
    EXPECT_EQ(mysender_test(message, recvfile, ws, pb, er), 1);
}
